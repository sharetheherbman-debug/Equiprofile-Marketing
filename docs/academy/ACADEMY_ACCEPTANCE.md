...
**Reason:** 63 lesson-level source-to-claim factual reviews remain unresolved, and several authenticated role workflows still require complete acceptance evidence.
...
| **Specific factual acceptance** | **BLOCKED** | Current factual evidence register records **63 lessons as `SOURCE_MAPPED_REQUIRES_SPECIFIC_CLAIM_REVIEW`**, 39 as `CLAIM_REVIEWED_AND_ACCEPTED`, and 3 as `NOT_MATERIAL_FACT_CHECK_REQUIRED`. Source mapping is not factual acceptance. |
| Safety/clinical/legal/numerical claim acceptance | **BLOCKED where included in the 63** | Material claims must be source-to-claim reviewed or rewritten as appropriately individualised/professional-guidance principles. |
...
- `CLAIM_REVIEWED_AND_ACCEPTED`: **39**
- `SOURCE_MAPPED_REQUIRES_SPECIFIC_CLAIM_REVIEW`: **63**
- Release-blocking unresolved specific-claim reviews: **63**

Do **not** replace these unresolved states with generated or inferred PASS values. The remaining 63 reviews are the principal Academy factual task for the next completion pass.
...
1. All 63 unresolved source-to-claim lesson reviews are explicitly completed and resulting corrections pass the full curriculum suite.
