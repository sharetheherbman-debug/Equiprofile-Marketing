...
**Reason:** 65 lesson-level source-to-claim factual reviews remain unresolved, and several authenticated role workflows still require complete acceptance evidence.
...
| **Specific factual acceptance** | **BLOCKED** | Current factual evidence register records **65 lessons as `SOURCE_MAPPED_REQUIRES_SPECIFIC_CLAIM_REVIEW`**, 37 as `CLAIM_REVIEWED_AND_ACCEPTED`, and 3 as `NOT_MATERIAL_FACT_CHECK_REQUIRED`. Source mapping is not factual acceptance. |
| Safety/clinical/legal/numerical claim acceptance | **BLOCKED where included in the 65** | Material claims must be source-to-claim reviewed or rewritten as appropriately individualised/professional-guidance principles. |
...
- `CLAIM_REVIEWED_AND_ACCEPTED`: **37**
- `SOURCE_MAPPED_REQUIRES_SPECIFIC_CLAIM_REVIEW`: **65**
- Release-blocking unresolved specific-claim reviews: **65**

Do **not** replace these unresolved states with generated or inferred PASS values. The remaining 65 reviews are the principal Academy factual task for the next completion pass.
...
1. All 65 unresolved source-to-claim lesson reviews are explicitly completed and resulting corrections pass the full curriculum suite.
