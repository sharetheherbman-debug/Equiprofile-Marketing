...
if (mode !== "fresh") {
  throw new Error("This command is fresh-database-only. Invoke with --mode=fresh.");
}
if (!databaseUrl) {
...
try {
  const [[schemaRow]] = await connection.query<Array<{ schemaName: string }>>("SELECT DATABASE() AS schemaName");
  ...
  execFileSync("npx", ["tsx", "scripts/build-migration-replay-fixture.ts"], {
    cwd: process.cwd(),
    stdio: "inherit",
  });
  ...
  const provisionedNames = new Set(provisionedTables.map((table) => table.tableName));
  const missingManifestTables = (manifest.tables ?? [])
    .map((table) => table.name)
    .filter((name) => !provisionedNames.has(name));
