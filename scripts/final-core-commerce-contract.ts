...
function statementsFromContract(contract: string): string[] {
  return contract
    ...
    .filter((statement) => /(?:CREATE|ALTER)\s+TABLE|CREATE\s+INDEX/i.test(statement));
}
...
export function getFinalCoreCommerceContractEvidence() {
  ...
  if (!statements.length) {
    throw new Error("Final-Core Commerce contract contains no executable schema statements.");
  }
