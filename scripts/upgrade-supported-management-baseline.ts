...
if (!isSupportedManagementMode && !isExactLegacyAdoptionMode) {
  throw new Error("This command is restricted to --mode=supported-management or --mode=exact-legacy-adoption.");
}
const backupReference = argument("--owner-backup-reference");
if (isExactLegacyAdoptionMode && !backupReference) {
  throw new Error("Exact legacy adoption requires --owner-backup-reference=<owner-controlled backup identifier>.");
}
...
const manifest = JSON.parse(readFileSync(resolve("docs/final-core-schema-manifest.json"), "utf8")) as {
  fingerprint?: string;
};
if (!manifest.fingerprint) throw new Error("Final-Core schema manifest has no fingerprint.");

function inspectReadOnly() {
  const result = spawnSync(
    "npx",
    ["tsx", "scripts/audit-migration-baseline.ts", `--database-url=${databaseUrl}`],
    { cwd: process.cwd(), encoding: "utf8", env: process.env },
  );
  if (result.error) throw result.error;
  const output = String(result.stdout ?? "");
  if (!output.trim()) {
    throw new Error(`Read-only schema inspector returned no report: ${String(result.stderr ?? "")}`);
  }
  return JSON.parse(output) as { classification?: string; differences?: Record<string, unknown> };
}
...
process.env.ADMIN_UNLOCK_PASSWORD ??= "supported-management-upgrade-process-only";
...
try {
  ...
  const managementBaselineContract = await applySupportedManagementBaselineForwardContract(connection);
  ...
  console.log(JSON.stringify({
    ...
    ownerBackupReference: isExactLegacyAdoptionMode ? backupReference : null,
