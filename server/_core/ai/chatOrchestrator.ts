 ...
 * work. Production Core uses GenX only; a missing GenX configuration produces
 * a truthful setup-needed state rather than a vendor fallback.
 */
 ...
 *
 * Returns a ChatSetupNeededError if GenX is not configured, so callers can
 * surface a clear setup message rather than a generic 500.
 */
...
/**
 * Returns true if the required GenX text provider is configured.
