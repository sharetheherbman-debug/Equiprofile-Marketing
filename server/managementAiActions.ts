 ...
 */
export async function executeManagementAiAction(input: { userId: number; confirmed: boolean; action: unknown }): Promise<ManagementAiActionResult> {
  ...
  if (action.type === "CREATE_TASK" || action.type === "CREATE_REMINDER") {
    ...
    await db.logActivity({ userId: input.userId, action: isReminder ? "ai_reminder_created" : "ai_task_created", entityType: "task", entityId: id });
  ...
  await db.logActivity({ userId: input.userId, action: "ai_calendar_item_created", entityType: "event", entityId: id });
